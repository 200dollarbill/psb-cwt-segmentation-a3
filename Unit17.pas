...
begin
  Self.FPi_pow_neg_0_25 := 0.75112554446; // Corrected value for 1 / (Pi^0.25)
...
const
  // This constant defines how wide the analysis range is around the center scale.
  // 0.75 means we will analyze a range of +/- 75% around the trackbar's value.
  // For example, if the trackbar scale is 100, the range will be [25, 175].
  SCALE_RANGE_PERCENT = 0.75;
var
  signalData: TArray<Double>;
  cwt_magnitude: TArray<TArray<Double>>;
  ...
  i_cwt, j_cwt: Integer;
  cwt_val: Double;
  a_min, a_max, a_step, center_scale: Double;
  f_min, f_max: Double;
begin
  if SignalTD.Count = 0 then
  begin
    LogMsg('No signal data to process. Please generate or load a signal.');
    Exit;
  end;

  LogMsg('Starting interactive CWT process...');
  ...
  // --- NEW INTERACTIVE SCALE LOGIC ---
  // 1. Get the center scale from the trackbar.
  // We divide by 100 to get a more manageable range of scale values.
  center_scale := ScaleTrackBar.Position / 100.0;
  if center_scale <= 0 then center_scale := 0.01; // Prevent division by zero

  // 2. Calculate the min and max scale for the CWT based on the center scale.
  a_min := center_scale * (1.0 - SCALE_RANGE_PERCENT);
  a_max := center_scale * (1.0 + SCALE_RANGE_PERCENT);
  if a_min <= 0 then a_min := 0.001; // Scale must be positive

  // 3. Calculate the frequency range that corresponds to our new scale range.
  f_max := CWT_F0 / a_min; // Highest frequency (from smallest scale)
  f_min := CWT_F0 / a_max; // Lowest frequency (from largest scale)
  // --- END OF NEW LOGIC ---

  cwt_w0 := 2 * Pi * CWT_F0;
  if CWT_ROW_COUNT > 1 then
    a_step := (a_max - a_min) / (CWT_ROW_COUNT - 1)
  else
    a_step := 0;
  ...
  begin
    current_a := a_min + scale_loop_idx * a_step;
    if current_a <= 0 then continue; // Skip if scale is not positive
    current_b := 0.0;
  ...
  // ... (The rest of the procedure for drawing the Bitmap is unchanged) ...
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if (xMax - xMin) > 1e-9 then
          RainbowColor((cwt_val - xMin) / (xMax - xMin), Row[i_bmp].rgbtRed, Row[i_bmp].rgbtGreen, Row[i_bmp].rgbtBlue)
        else
          RainbowColor(0, Row[i_bmp].rgbtRed, Row[i_bmp].rgbtGreen, Row[i_bmp].rgbtBlue);
      end;
    end;
  ...
  // Update labels to show the DYNAMIC frequency range being analyzed
  Label9.Caption := Format('%.1f Hz', [f_max]); // Highest frequency at the top
  Label10.Caption := Format('%.1f Hz', [f_min]); // Lowest frequency at the bottom
  LogMsg('CWT process finished.');
end;
procedure TForm17.LoadCSVClick(Sender: TObject);
var
  i, pointsToRead: Integer;
  textFileHandle: TextFile;
  colN, colV2: Extended; // These variables are needed for the ReadLn structure
  signalY: Extended;
...
begin
  ...
  begin
    ...
    try
      ...
      // Skip the first two header lines, as in your original code
      if not Eof(textFileHandle) then ReadLn(textFileHandle);
      if not Eof(textFileHandle) then ReadLn(textFileHandle);
      ...
      i := 0;
      // Use the original ReadLn to read space-separated data
      while (not Eof(textFileHandle)) and (i < pointsToRead) do
      begin
        // This line reads three numeric values from the line, assuming they are separated by spaces.
        // The signal value is expected to be the second one (signalY).
